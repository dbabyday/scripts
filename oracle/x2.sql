...
BEGIN
	...
	select   name, to_char(dbid), to_char(created,'YYYYMMDD HH24MISS'),   log_mode,   cdb, to_char(con_id), to_char(con_dbid)
	into   l_dbname,       l_dbid,        l_created,                      l_log_mode, l_cdb,       l_con_id,        l_con_dbid
	from   v$database;

	select   host_name,   version,   edition
	into   l_host_name, l_version, l_edition
	from   v$instance;

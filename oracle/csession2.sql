...
prompt ;
prompt substitution variable 1 is for CLIENT_OS_PROCESS_ID
column sid new_value _CLIENT_OS_PROCESS_ID noprint;
...
BEGIN
	...
	from   v$session
	where  process='&&_CLIENT_OS_PROCESS_ID';

	IF l_qty=0 THEN
		dbms_output.put_line('There is no session connected with CLIENT_IDENTIFIER '||to_char(&&_CLIENT_OS_PROCESS_ID));
	ELSE
		FOR x IN (
			select    s.sid
			        , s.serial#
			...
			where     s.process='&&_CLIENT_OS_PROCESS_ID'
		) LOOP
...
undefine 1
undefine _CLIENT_OS_PROCESS_ID

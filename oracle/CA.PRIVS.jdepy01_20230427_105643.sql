...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "CA"."PRIVS" AS
	...
	BEGIN
		...
		FOR x IN (  select   case grantable when 'YES' THEN 'GRANT '||listagg(privilege,', ') within group (order by privilege)||' ON '||in_owner||'.'||in_table_name||' TO '||grantee||' WITH GRANT OPTION;'
		                                    else            'GRANT '||listagg(privilege,', ') within group (order by privilege)||' ON '||in_owner||'.'||in_table_name||' TO '||grantee||';'
		                     end stmt
		            from     ca.dflt_privs
	...
	BEGIN
		...
		FOR s IN (  select   case when hierarchy='YES' and grantable='YES' then 'grant '||listagg(privilege,', ') within group (order by privilege)||' on "'||owner||'"."'||table_name||'" to "'||grantee||'" with hierarchy option with grant option;'
				                  when hierarchy='YES' and grantable='NO'  then 'grant '||listagg(privilege,', ') within group (order by privilege)||' on "'||owner||'"."'||table_name||'" to "'||grantee||'" with hierarchy option;'
				                  when hierarchy='NO'  and grantable='YES' then 'grant '||listagg(privilege,', ') within group (order by privilege)||' on "'||owner||'"."'||table_name||'" to "'||grantee||'" with grant option;'
				                  else 'grant '||listagg(privilege,', ') within group (order by privilege)||' on "'||owner||'"."'||table_name||'" to "'||grantee||'";'
				             end stmt
	...
	BEGIN
		...
		FOR s IN (  select   case when grantable='YES' then 'grant '||listagg(privilege,', ') within group (order by privilege)||' ("'||column_name||'") on "'||owner||'"."'||table_name||'" to "'||grantee||'" with grant option;'
				                  else 'grant '||listagg(privilege,', ') within group (order by privilege)||' ("'||column_name||'") on "'||owner||'"."'||table_name||'" to "'||grantee||'";'
				             end stmt
				    from     sys.dba_col_privs
				    where    grantee=in_role
				    group by owner

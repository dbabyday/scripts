...
DECLARE 
	-- user input
	@resultsUnits  CHAR(2) = 'MB', -- 'KB', 'MB', or 'GB'
...
-- set the dufault for @resultsUnits if a valid selection was not entered
IF (UPPER(@resultsUnits) <> 'KB') AND (UPPER(@resultsUnits) <> 'MB') AND (UPPER(@resultsUnits) <> 'GB')
	SET @resultsUnits = 'MB';

...
*          Shows size history.
*          Provides command to modify file size.
* 
* Notes:
*     1. Connect and run on CO-DB-042
*     2. Paste [usp_FileGrowth @where] from FILES_Info.sql
*        - OR - Enter the server, database, and file names
*     3. Get the version: EXECUTE [CentralAdmin].[dbo].[usp_FileGrowth] @getVersion = 1;
*
**************************************************************************************/
...
IF OBJECT_ID('dbo.usp_fileGrowth','P') IS NOT NULL
	DROP PROCEDURE dbo.usp_FileGrowth;
GO

CREATE PROCEDURE dbo.usp_FileGrowth
	@where      NVARCHAR(MAX) = '',
	@server     NVARCHAR(128) = '',
...
-- provide info if it is requested, or if no parameters were set
IF ( (@where = '') AND (@server = '' OR @database = '' OR @file = '') OR ( (@help != '') AND (UPPER(@help) != 'N') AND (UPPER(@help) != 'NO') ) )
BEGIN
	SET @message = '
...
*     2. Paste [usp_FileGrowth @where] from FILES_Info.sql
*        - OR - Enter the server, database, and file names
*     3. Get the version: EXECUTE [CentralAdmin].[dbo].[usp_FileGrowth] @version = 1;
* 
**************************************************************************************/';
	PRINT @message;
	RETURN;
...
BEGIN 
    ...
    SET @message = @message + 'You must paste the [usp_FileGrowth @where] string from FILES_Info.sql into @where OR enter values for @server, @database, and @file; but not both.';
...
BEGIN
    ...
    BEGIN
        SET @message = 'Incorrect format for @where.' + CHAR(13) + CHAR(10);
    ...
    BEGIN
        ...
        SET @message = @message + 'The specified server, [' + SUBSTRING(@where,18,CHARINDEX(''' AND databasename',@where) - 18) + '], is not in this table.';
        RAISERROR(@message,16,1);
        RETURN;
    END
    ...
    BEGIN
        ...
        SET @message = @message + 'The specified database, [' + SUBSTRING(@where,CHARINDEX('databasename = ',@where) + 16,CHARINDEX(''' AND DatabaseFileName',@where) - (CHARINDEX('databasename = ',@where) + 16)) + '], is not in this table.';
        RAISERROR(@message,16,1);
        RETURN;
    END
    ...
    BEGIN
        ...
        SET @message = @message + 'The specified file, [' + RIGHT(LEFT(@where,LEN(@where)-1),LEN(@where)-1 - (CHARINDEX(''' AND DatabaseFileName',@where) + 25)) + '], is not in this table.';
        RAISERROR(@message,16,1);
        RETURN;
    END
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            SET @message = 'Invalid server.' + CHAR(13) + CHAR(10);
            SET @message = @message + 'The specified server, [' + @server + '], is not in this table.';
            RAISERROR(@message,16,1);
            RETURN;
        END
    END
    ...
    BEGIN
        ...
        BEGIN
            SET @message = 'Invalid database.' + CHAR(13) + CHAR(10);
            SET @message = @message + 'The specified database, [' + @database + '], is not in this table.';
            RAISERROR(@message,16,1);
            RETURN;
        END
    END
    ...
    BEGIN
        ...
        BEGIN
            SET @message = 'Invalid file.' + CHAR(13) + CHAR(10);
            SET @message = @message + 'The specified file, [' + @file + '], is not in this table.';
            RAISERROR(@message,16,1);
            RETURN;
        END
    END

/**********************************************************************************************
* 
* FILES_SuggestedLogFileSize.sql
* 
* Author: James Lutsey
* Date: 2017-01-24
* 
* Purpose: Find log files that are less than twice the size of the largest index
* 
**********************************************************************************************/
...
-- temp table to hold info form xp_fixeddrives
...
-- cursor to loop through drives and get freespace and totalspace
...
IF @Result <> 0 EXEC sp_OAGetErrorInfo @FSO
...
OPEN curDrives;
	...
	BEGIN
		...
		IF @Result <> 0 EXEC sp_OAGetErrorInfo @DriveNameOut;
...
SELECT
	@selectedFreespace = [FreeSpace],
	@selectedCapacity  = [TotalSize]
FROM #Drives
WHERE [Drive] = @selectedDrive;
...
BEGIN
	...
	IF @selectedSize < @selectedFreeSpace
	BEGIN
		SET @growFileCmd = ''USE [master]; ALTER DATABASE '' + QUOTENAME(DB_NAME()) + '' MODIFY FILE ( NAME = N'''''' + @logFileName + '''''', ' + 
																									   ...
																									   'FILEGROWTH = '' + CAST(@selectedFilegrowth AS NVARCHAR(25)) + ''MB );'';
	END
	ELSE
		SET @growFileCmd = ''-- THERE IS NOT ENOUGH DRIVE SPACE TO GROW THE FILE TO THE RECCOMENDED SIZE'';
END
ELSE
	SET @growFileCmd = ''-- size is okay for log file on ['' + DB_NAME() + '']'';

INSERT INTO #LogFiles ([DatabaseName],[DataFilesSize_MB],[LargestIndex_MB],[LogFileSize_MB],[SuggestedMinSize_MB],[Drive],[Capacity_MB],[Freespace_MB],[GrowFile])
VALUES (DB_NAME(),@dataFilesSize_MB,@largestIndex_MB,@logFileSize_MB,@selectedSize,@selectedDrive,@selectedCapacity,@selectedFreeSpace,@growFileCmd)
');

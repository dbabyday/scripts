...
DECLARE 
	 @database_type AS VARCHAR(50)= 'Global' -- Regional or Transfer ** Change accordingly **

INSERT INTO @TablesRequired(
	 TableSchema
    ,TableName)
VALUES 
	    ('GlobalReference','ReplicationStatus')
...
IF EXISTS (SELECT 1 FROM GlobalReference.RegionalSqlServer WHERE InitialCatalog=DB_NAME())
	BEGIN
...
IF (@vcGetDatabaseType='' OR @vcGetDatabaseType IS NULL)
	BEGIN
...
DECLARE curAddArticle CURSOR 	
	...
	WHILE (@@FETCH_STATUS=0)
		BEGIN
			...
			FETCH NEXT FROM curGenerateArticleScript INTO @vcTableSchema,@vcTableName
				...
				BEGIN
					SET @nvcSQL=
								...
								exec sp_dropsubscription 
									 @publication='''+@vcPublication_Name+''''
									+',@article='''+@vcTableName+''''
									+', @subscriber=N''all'';'

...
--Jeff add:  use below 2 queries to find plan handle for a stored proc and get the cached execution plan
...
WHERE  est.objectid = object_id ('dbo.usp_LaborEventAll_Select')
  AND  est.dbid     = db_id('gsf2_amer_prod') 


  select * from sys.dm_exec_query_plan(0x05001000F365AA61406121B4000000000000000000000000)
...
SELECT TOP ( 25 )
       ...
       -- deps.total_logical_reads AS [TotalLogicalReads] ,
        ...
        ISNULL(deps.execution_count / DATEDIFF(minute, deps.cached_time,
                                           GETDATE()), 0) AS [Calls/Minute] ,
        deps.total_elapsed_time ,
        deps.total_elapsed_time / deps.execution_count / 1000 AS [avg_elapsed_time_ms] ,
        deps.cached_time
FROM    sys.procedures AS p
        INNER JOIN sys.dm_exec_procedure_stats
                       AS deps ON p.[object_id] = deps.[object_id]
WHERE   deps.database_id = DB_ID()
ORDER BY deps.total_logical_reads / deps.execution_count DESC ;
...
-- Indexes with NO Page Compression
SELECT DISTINCT

...
DECLARE @database_type AS VARCHAR(50) = 'Global' -- Global, Regional or Transfer ** Change accordingly **
	...
	BEGIN
		...
		WHILE
			...
			BEGIN
				...
				IF (@database_type = 'Regional'
				    AND @vcGetDatabaseType = 'Regional') 
					BEGIN
				...
				IF (@database_type = 'Global'
				    AND @vcGetDatabaseType = 'Global') 
					BEGIN
						SET @nvcSql = '
										exec sp_addarticle 
												@publication=N''' + @vcPublication_Name + '''
						...
						']''
											   ,@upd_cmd = N''SCALL [dbo].[sp_MSupd_' + @vcTableSchema + @vcTableName + ']'';
										'
						IF ( (@vcPublication_Name LIKE '%APAC%'
							 OR @vcPublication_Name LIKE '%XIAM%'
							 OR @vcPublication_Name LIKE '%CHINA%')
							AND (@vcGlobalFilterValue IS NOT NULL
								OR @vcGlobalFilterValue <> '') ) 
							BEGIN
								SET @nvcSQL = @nvcSQL + '
				...
				IF (@database_type = 'Transfer'
				    AND @vcGetDatabaseType = 'Transfer') 
					BEGIN
						SELECT TOP 1 
						...
						FROM 
							...
							INNER JOIN dbo.syspublications AS p
								ON a.pubId = p.pubId
						WHERE p.name = @vcPublication_Name
